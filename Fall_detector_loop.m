...
%%video_size=222;
mode=0;   % change to 1 for enhancment
video_size=6;   % 

for p=1:video_size;
    
%      vid = VideoReader( 'C:\\Users\JaroslawJ\Desktop\MATALB_ImageTesting\Recived\Tester_video.mp4' );
%      video_name= [ 'C:\\Users\JaroslawJ\Desktop\MATALB_ImageTesting\Recived\Video_' num2str(p,'%02d'),'.avi'];
%      video_name= [ 'C:\\Users\JaroslawJ\Desktop\MATALB_ImageTesting\Recived\ENHANCMENT\Video_' num2str(p,'%02d'),'.avi'];
       video_name= [ 'C:\Users\JaroslawJ\Desktop\MATALB_ImageTesting\Recived\PRESENTATION\Video_' num2str(p,'%02d'),'.avi'];
...
%      fgMask = step(detector,enhanced_frame);  
    ...
    if ( ratio_array_vision(i)< 0.65&& ratio_array_vision(i)>0.2)    % RATIO_2 WEIGHT 2
        ...
        if (no_movment_check > 30 )
             fall_array(i)= fall_array(i) + 5;
        ms = [ 'OBJECT IS NOT MOVING: ' num2str(i,'%02d') '  FLAG=', num2str(fall_array(i),'%02d')];
        disp(ms)
        mhi_flag=false; % turn off ratio part after mhi method
        email_flag=true;
        end   
    end 
  ...
  if ( speed_array(i) > 1.85)    % SPEED WEIGTH 7 
    mhi_flag=true;
    fall_array(i)= fall_array(i) + 7;   % add 7   
...
if(email_flag)
%---------------------
% emial code implemented in fall_detector function
